...
var Readme = require('./types/readme');
...
HTMLBaseConverter.prototype.parseChapters = function() {
    ...
    // Create README
    var readme = Readme.generate('md', that.documentTitle ,that._projectFolder);
    ...
    if (!that.chapters.length) {
        readme.content = that._html;
    }
    ...
    that.chapters.forEach(function(chapter) {
        chapter.generateFilename('md', that.prefixFilenames);
    });
...
HTMLBaseConverter.prototype.detectTitleTags = function() {
    ...
    tags.forEach(function(tag) {
        if (titleTags.length === that.titleDepth) return;
...
HTMLBaseConverter.prototype.parseHTML = function(html, level, parent) {
    ...
    if (!tagDetector.test(parts[0])) {
        ...
        if (!!preface) parent.content = preface;
    }
    ...
    parts.forEach(function(part) {
        ...
        if (tagDetector.test(part)) {
            ...
            chapter = new Chapter(that._projectFolder);
...
HTMLBaseConverter.prototype.writeSummary = function() {
    ...
    that.chapters.forEach(function(chapter) {
        ...
        // Add summary entry
        var entry = padding+'* ['+chapter.title+']('+chapter.summaryPath+')\n';
...
HTMLBaseConverter.prototype.writeFiles = function(first_argument) {
    ...
    that.chapters.forEach(function(chapter) {
        console.log('Writing file: '+chapter.filepath);

        // Try to create directory
        Q.nfcall(fs.stat, chapter.path)
        .fail(function function_name(argument) {
            return Q.nfcall(fs.mkdir, chapter.path);
        })
        .fin(function() {
            // Write converted file
            Q.nfcall(fs.writeFile, chapter.filepath, chapter.markdown)
            .fail(that.handleError);
        });
    });
};

HTMLBaseConverter.prototype.handleError = function(err) {
