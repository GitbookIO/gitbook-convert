...
Chapter.prototype.generateFilename = function(ext, prefixFilenames) {
    var that = this;
    that.path = '';

    var parent = that.parent;
    var siblings = parent? _.without(parent.children, that) : [];
    while (!!parent) {
        that.path = path.join(normalizeFilename(parent, prefixFilenames), that.path);
        parent = parent.parent;
    }

    if (!!that.children.length) {
        that.path = path.join(that.path, normalizeFilename(that, prefixFilenames));
        that.filename = 'README';
    }
    else {
        that.filename = normalizeFilename(that, prefixFilenames);
    }

    // Suffix filename if a sibling has the same name
    siblings.forEach(function(sibling) {
        var siblingFilename = path.basename(sibling.filename, '.'+ext);
        if (siblingFilename === that.filename)
            that.filename += '.'+that.num;
    });

    that.filename = that.filename+'.'+ext;

    // Keep local reference for summary
    that.summaryPath = path.join(that.path, that.filename);
    // Actual paths on FS
    that.path = path.join(that.rootDir, that.path);
    that.filepath = path.join(that.path, that.filename);
};
