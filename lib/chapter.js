...
Chapter.prototype.resolveLinks = function(siblings) {
    ...
    $('a').each(function() {
        ...
        // Look for ref in the other chapters
        siblings.forEach(function(sibling) {
            // Link to a filename
            if (href.slice(1) === sibling.titleId) {
                link = './'+sibling.filename;
            }
            // Link to an element inside the chapter
        ...
        // Delete link by replacing it by a <p> tag if no reference exists
        else {
            var $replacement = $('<p></p>');
